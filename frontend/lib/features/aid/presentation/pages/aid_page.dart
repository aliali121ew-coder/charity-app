                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        ...
                        showModalBottomSheet(
                          ...
                          builder: (_) => _AddAidSheet(
                            isDark: isDark,
                            onSave: (newAid) {
                              ref.read(aidProvider.notifier).addAid(newAid);
                              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                content: Text('✓ تمت إضافة المساعدة بنجاح', style: GoogleFonts.cairo(color: Colors.white)),
...
class _AddAidSheet extends StatefulWidget {
  final bool isDark;
  final Function(AidModel) onSave;

  const _AddAidSheet({required this.isDark, required this.onSave});

  @override
  State<_AddAidSheet> createState() => _AddAidSheetState();
}

class _AddAidSheetState extends State<_AddAidSheet> {
  final _formKey = GlobalKey<FormState>();
  final _nameCtrl = TextEditingController();
  final _amountCtrl = TextEditingController();
  final _notesCtrl = TextEditingController();
  AidType _selectedType = AidType.financial;
            ...
            child: Column(
              ...
              children: [
                ...
                _AddAidFormField(
                  ...
                  hint: 'أدخل اسم المستفيد الكامل',
                  icon: Icons.person_rounded,
                  isDark: isDark,
                  validator: (value) => value == null || value.trim().isEmpty ? 'يرجى إدخال اسم المستفيد' : null,
                ),
                const SizedBox(height: 12),
                ...
                _AddAidFormField(
                  ...
                  validator: (value) {
                    if (value == null || value.trim().isEmpty) return 'يرجى إدخال القيمة';
                ...
                _AddAidFormField(
                  ...
                  hint: 'أدخل أي تفاصيل أو ملاحظات أخرى',
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      if (_formKey.currentState?.validate() ?? false) {
                        ...
                        final newAid = AidModel(
                          id: 'a${DateTime.now().millisecondsSinceEpoch}',
                          referenceNumber: 'AID-2026-${100 + DateTime.now().second}',
                        ...
                        widget.onSave(newAid);
                        Navigator.pop(context);
                      }
                    },
                    icon: const Icon(Icons.check_rounded, size: 18),
                    label: Text('حفظ المساعدة', style: GoogleFonts.cairo(fontSize: 14, fontWeight: FontWeight.w700)),
...
class _AddAidFormField extends StatelessWidget {
  final String label;
  final String hint;
  final IconData icon;
