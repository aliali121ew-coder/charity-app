...
class AidNotifier extends StateNotifier<AidState> {
  final MockAidRepository _repo;

  AidNotifier(this._repo)
      : super(AidState(
          all: _repo.getAll(),
  ...
  void addAid(AidModel record) {
    _repo.add(record);
    final updated = _repo.getAll();
    state = state.copyWith(
      all: updated,
      filtered: _applyFilters(query: state.query, type: state.typeFilter, status: state.statusFilter),
    );
  }
}

final aidProvider =
    StateNotifierProvider<AidNotifier, AidState>((ref) {
  return AidNotifier(MockAidRepository());
});
